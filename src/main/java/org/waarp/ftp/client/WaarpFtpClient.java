...
public class WaarpFtpClient {
  ...
  public WaarpFtpClient(String server, int port,
                        String user, String pwd, String acct, boolean isPassive,
                        int ssl, int controlTimeout,
                        int timeout) {
    this.server = server;
    this.port = port;
    this.user = user;
    ...
    this.ftpClient
        .addProtocolCommandListener(new PrintCommandListener(new PrintWriter(
            System.out), true));
  }
  ...
  public boolean connect() {
    ...
    try {
      ...
      if ((!isActive) && this.ftpClient.getDataConnectionMode() ==
  ...
  public void logout() {
    try {
      ...
      if (this.ftpClient.getDataConnectionMode() ==
          FTPClient.ACTIVE_LOCAL_DATA_CONNECTION_MODE) {
        try {
          this.ftpClient.disconnect();
        } catch (IOException f) {
